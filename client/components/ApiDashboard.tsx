  ...
  const refreshApiStatus = async (apiName: string) => {
    try {
      ...
      setApiStatuses(prev => prev.map(api => 
        api.name === apiName 
          ? { ...api, status: isHealthy ? 'healthy' : 'unhealthy', health, lastChecked: new Date() }
          : api
      ));
            ...
            {apiStatuses.map((api) => (
              <div
                ...
                className={cn(
                  ...
                  selectedApi === api.name ? "border-primary bg-primary/5" : "hover:bg-muted/50",
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex space-x-1">
                    <div className={cn("w-3 h-3 rounded-full", 
                      api.health.liveness?.status === 'ok' ? 'bg-green-500' : 'bg-red-500'
                    )} title={`Liveness: ${api.health.liveness?.status || 'unknown'}`} />
                    <div className={cn("w-3 h-3 rounded-full",
                      api.health.readiness?.status === 'ready' ? 'bg-green-500' : 'bg-red-500'
                ...
                <Tabs defaultValue="liveness" className="space-y-4">
                  ...
                  <TabsContent value="liveness" className="space-y-4">
                    <div className="rounded-lg border p-4">
                      ...
                      <div className="text-sm space-y-2">
                        <p><strong>Status:</strong> {api.health.liveness?.status || 'unknown'}</p>
                        {api.health.liveness?.uptime && (
                          <p><strong>Uptime:</strong> {Math.floor(api.health.liveness.uptime)} seconds</p>
                        )}
                        {api.health.liveness?.timestamp && (
                          <p><strong>Timestamp:</strong> {new Date(api.health.liveness.timestamp).toLocaleString()}</p>
                        )}
                      </div>
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="readiness" className="space-y-4">
                    <div className="rounded-lg border p-4">
                      ...
                      <div className="text-sm space-y-2">
                        <p><strong>Status:</strong> {api.health.readiness?.status || 'unknown'}</p>
                        {api.health.readiness?.timestamp && (
                          <p><strong>Timestamp:</strong> {new Date(api.health.readiness.timestamp).toLocaleString()}</p>
                        )}
                        {api.health.readiness?.checks && (
                          <div className="mt-3">
                            <p className="font-medium mb-2">Health Checks:</p>
                            <div className="space-y-1">
                              {Object.entries(api.health.readiness.checks).map(([check, status]) => (
